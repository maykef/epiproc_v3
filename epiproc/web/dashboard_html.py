...
def _costing_dashboard_script(sym: str) -> str:
    """Customer-facing 'Costing' tab. The supplier's offers are the structure:
    each one is its filename and the date it was submitted, and opening it shows
    the whole book of prices that offer recorded. A single offer is not a history,
    so the book is shown on its own until a second offer exists. Data is embedded
    via _js_json (script-context safe) and every product-derived string is escaped
    on insert (no inline handlers — the nav lazily calls data-init="initCosting").
    Guarded so an old DB without the costing tables never breaks the dashboard."""
    try:
    ...
    return (
        ...
        "return isNaN(d)?String(s):d.toLocaleDateString()+' '+d.toLocaleTimeString();}\n"
        "  var D=(window.COSTING_DATA&&window.COSTING_DATA.products)||[];\n"
        "  var OF=(window.COSTING_DATA&&window.COSTING_DATA.offers)||[];\n"
        "  if(!D.length&&!OF.length){root.innerHTML='<div class=\"card\"><p style=\"color:var(--muted)\">"
        "No costings yet. An administrator creates them under Admin \\u2192 Costing.</p></div>';return;}\n"
        # The book of prices one offer recorded, or the current costing set.
        "  function bookRows(items){\n"
        "    var b='<table><thead><tr><th>Product</th><th>EAN</th><th>Total cost</th>'\n"
        "      +'<th>Our Price</th><th>Our GP%</th><th>Customer GP%</th><th>Customer price</th>'\n"
        "      +'</tr></thead><tbody>';\n"
        "    items.forEach(function(p){\n"
        "      b+='<tr><td>'+esc(p.name)+(p.version==null?' <span style=\"color:var(--muted)\">(no final costing)</span>':' <span style=\"color:var(--muted)\">v'+esc(p.version)+'</span>')+'</td>'\n"
        ...
        "      var r=p.results;\n"
        "      if(r){\n"
        "        b+='<tr><td colspan=\"7\"><details><summary style=\"cursor:pointer;color:var(--accent)\">Breakdown</summary>'\n"
        ...
        "  }\n"
        # The offers are the index: you see the files submitted, newest first, and
        # choose one — its prices drop down on click and fold away again.
        "  var h='';\n"
        "  if(OF.length){\n"
        "    h+='<div class=\"card\"><h2>Offers</h2>';\n"
        "    OF.forEach(function(f){\n"
        "      h+='<details style=\"border-top:1px solid var(--line);padding:10px 0\">'\n"
        "        +'<summary style=\"cursor:pointer\"><strong>'+esc(f.filename||'')+'</strong>'\n"
        "        +' <span style=\"color:var(--muted)\">'+esc(fmtDate(f.uploaded_at))+'</span></summary>'\n"
        "        +'<div style=\"margin-top:10px\">'+bookRows(f.items||[])+'</div></details>';\n"
        "    });\n"
        "    h+='</div>';\n"
        "  }else{\n"
        "    h+='<div class=\"card\"><h2>Product costing (latest final version)</h2>'+bookRows(D)+'</div>';\n"
        "  }\n"
        "  root.innerHTML=h;\n"
        "}\n"
        "</script>"
